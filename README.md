...
- Installer UX notes and modernization path: `docs/INSTALLER-UX.md`.
...
- **MSI error 1307 / Config.Msi permissions**
  - Error: `Could not set file security for file 'D:\\Config.Msi\\*.rbf'. Error: 1307`
  - Fix: run installer as Administrator and launch it from a local NTFS path like `C:\\Users\\<you>\\Downloads` (not root of secondary drives). If needed, grant `SYSTEM` Full Control on the affected drive root and retry.


- **electron-builder Access is denied (win-unpacked\d3dcompiler_47.dll)**
  - Error: `remove ...\win-unpacked\d3dcompiler_47.dll: Access is denied`
  - Fix: close any running "Local LLM Workbench"/Electron process and File Explorer windows pointing to `app\release`, then rerun `installer\build-installer.ps1` as Administrator.

- **WiX BAL extension errors (WIX0200/WIX0201)**
  - Error: `WixStandardBootstrapperApplication ... unhandled extension element/attribute`
  - Fix: ensure build uses `-ext WixToolset.Bal.wixext` (already included in `installer/build-installer.ps1`) and that WiX Toolset v4+ is installed.
