...
function writeEnv() {
  ...
  const env = [
    ...
    `LLAMA_CPP_HOST_PORT=${cfg.llamaCppPort}`,
...
ipcMain.handle("setConfig", (_e, next) => {
  fs.writeFileSync(configPath, JSON.stringify(next, null, 2));
  writeEnv();
  return next;
});
ipcMain.handle("startStack", async () => {
  writeEnv();
  const output = await runCompose(["up", "-d", "--build"]);
  const cfg = readConfig();
  await waitForWebUI(cfg.webuiPort);
...
ipcMain.handle("stopStack", () => runCompose(["down"]));
ipcMain.handle("getLogs", (_e, service) => runCompose(["logs", "--tail", "100", service]));
ipcMain.handle("webuiUrl", () => `http://127.0.0.1:${readConfig().webuiPort}`);
ipcMain.handle("openExternal", (_e, url) => shell.openExternal(url));
ipcMain.handle("getRepoRoot", () => path.resolve(__dirname, "../.."));
